...
{
    public partial class frmGroup : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        #region VARIABLE
        ...
        #region Constructor
        public static frmGroup p_instance = null;
        ...
        private void Mapping()
        {
            ...
            dtRecord = GTSysOne.Class.MasterFile.clsMas_Group.ShowTable(GTSysOne.Class.MasterFile.clsMas_Group.Value(0, 3, Id));
            if (dtRecord.Rows.Count > 0)
            {
                ...
                if (EnformStatus == GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.View)
                {
                    this.Enable(false);
                }
            }
            if (EnformStatus == GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.Add)
            {
                this.Enable(true);
                this.txtNumbering.Text = GTSysOne.Class.Utility.clsUtility.Numbering(GTSysOne.Class.Utility.clsGlobal.myFunction.Group);
            }
        ...
        private void FORM_LOAD(object sender, System.EventArgs e)
        {
            this.Mapping();
            this.EventHandler();
            GTSysOne.Class.Utility.clsUtility.Language(this, typeof(GTSysOne.Gui.MasterFile.frmGroup).Assembly);
        }
        ...
        private void SEARCH_PRESSED(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                string ResultId = System.Convert.ToString(GTSysOne.Class.MasterFile.clsMas_Group.ScalarValue(GTSysOne.Class.MasterFile.clsMas_Group.Value(0, 4, System.Convert.ToString((sender as DevExpress.XtraEditors.ButtonEdit).Text))));
                if (ResultId.CompareTo(string.Empty) != 0)
                {
                    EnformStatus = GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.Edit;
                    Id = ResultId;
        ...
        private void CLICK(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            switch (e.Item.Name)
            {
                case "barNew":
                    ...
                    EnformStatus = GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.Add;
                    Id = string.Empty;
                    this.EventHandler(false);
                    this.Clear();
                    this.Mapping();
                ...
                case "barSave":
                    ...
                    if (GTSysOne.Class.Utility.clsGlobal.Instance().No_Error_Result)
                    {
                        ...
                        Id = GTSysOne.Class.MasterFile.clsMas_Group.Save(
                            GTSysOne.Class.MasterFile.clsMas_Group.Value(
                            (EnformStatus == GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.Add ? 1 : 2)
                            , 0
                            , Id
                            , (EnformStatus == GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.Add ? (this.txtNumbering.Enabled == false ? GTSysOne.Class.Utility.clsUtility.Numbering(GTSysOne.Class.Utility.clsGlobal.myFunction.Group, true) : this.txtNumbering.Text) : this.txtNumbering.Text)
                            ...
                            , System.Convert.ToString((int)GTSysOne.Class.Utility.clsGlobal.myFunction.Group)
                            , GTSysOne.Class.Utility.clsGlobal.Instance().UserId
                            , string.Empty
                            , this.txtName.Text
                            , "0"));
                        GTSysOne.Gui.Dashboard.MasterFile.frmGroup.CallFromMain("Refresh", Id, false, 1, Id);
                        this.Clear();
                        this.Mapping();
                    }
                    break;
                case "barFirst":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "2", GTSysOne.Class.Utility.clsGlobal.myFunction.Group);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.View)
                        {
                    ...
                    break;
                case "barPrevious":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "0", GTSysOne.Class.Utility.clsGlobal.myFunction.Group);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.View)
                        {
                    ...
                    break;
                case "barNext":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "1", GTSysOne.Class.Utility.clsGlobal.myFunction.Group);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.View)
                        {
                    ...
                    break;
                case "barLast":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "3", GTSysOne.Class.Utility.clsGlobal.myFunction.Group);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmGroup.enFormStatus.View)
                        {
