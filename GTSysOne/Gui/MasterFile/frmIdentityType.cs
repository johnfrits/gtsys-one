...
{
    public partial class frmIdentityType : DevExpress.XtraBars.Ribbon.RibbonForm
    {
        #region VARIABLE
        ...
        #region Constructor
        public static frmIdentityType p_instance = null;
        public static frmIdentityType Instance
        {
            get
            {
                if (p_instance == null)
                {
                    p_instance = new frmIdentityType();
                }
                else
                {
        ...
        private void Mapping()
        {
            ...
            dtRecord = GTSysOne.Class.MasterFile.clsMas_IdentityType.ShowTable(GTSysOne.Class.MasterFile.clsMas_IdentityType.Value(0, 3
                , Id
                , string.Empty
                , false
                , false
            ...
            if (dtRecord.Rows.Count > 0)
            {
                ...
                this.txtName.Text = System.Convert.ToString(dtRecord.Rows[0]["identity_type"]);
                this.chkActive.Checked = !System.Convert.ToBoolean(dtRecord.Rows[0]["void"]);


                if (EnformStatus == GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.View)
                {
                    this.Enable(false);
                }
            }
            if (EnformStatus == GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.Add)
            {
                this.Enable(true);
                this.txtNumbering.Text = GTSysOne.Class.Utility.clsUtility.Numbering(GTSysOne.Class.Utility.clsGlobal.myFunction.IdentityType);
            }
        ...
        private void FORM_LOAD(object sender, System.EventArgs e)
        {
            this.Mapping();
            this.EventHandler();
            GTSysOne.Class.Utility.clsUtility.Language(this, typeof(GTSysOne.Gui.MasterFile.frmIdentityType).Assembly);
        }
        ...
        private void SEARCH_PRESSED(object sender, DevExpress.XtraEditors.Controls.ButtonPressedEventArgs e)
        {
            ...
            {
                string ResultId = System.Convert.ToString(GTSysOne.Class.MasterFile.clsMas_IdentityType.ScalarValue(GTSysOne.Class.MasterFile.clsMas_IdentityType.Value(0
                    , 4
                ...
                {
                    EnformStatus = GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.Edit;
                    Id = ResultId;
        ...
        private void CLICK(object sender, DevExpress.XtraBars.ItemClickEventArgs e)
        {
            ...
            switch (e.Item.Name)
            {
                case "barNew":
                    ...
                    EnformStatus = GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.Add;
                    Id = string.Empty;
                    this.EventHandler(false);
                    this.Clear();
                    this.Mapping();
                ...
                case "barSave":
                    ...
                    if (GTSysOne.Class.Utility.clsGlobal.Instance().No_Error_Result)
                    {
                        ...
                        Id = GTSysOne.Class.MasterFile.clsMas_IdentityType.Save(
                            GTSysOne.Class.MasterFile.clsMas_IdentityType.Value(
                            (EnformStatus == GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.Add ? 1 : 2)
                            , 0
                            , Id
                            , (EnformStatus == GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.Add ? (this.txtNumbering.Enabled == false ? GTSysOne.Class.Utility.clsUtility.Numbering(GTSysOne.Class.Utility.clsGlobal.myFunction.IdentityType, true) : this.txtNumbering.Text) : this.txtNumbering.Text)
                            ...
                            , System.Convert.ToString((int)GTSysOne.Class.Utility.clsGlobal.myFunction.IdentityType)
                            , GTSysOne.Class.Utility.clsGlobal.Instance().UserId
                            , string.Empty
                            , this.txtName.Text
                            , "0"));
                        GTSysOne.Gui.Dashboard.MasterFile.frmIdentityType.CallFromMain("Refresh", Id, false, 1, Id);
                        this.Clear();
                        this.Mapping();
                    }
                    break;
                case "barFirst":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "2", GTSysOne.Class.Utility.clsGlobal.myFunction.IdentityType);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.View)
                        {
                    ...
                    break;
                case "barPrevious":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "0", GTSysOne.Class.Utility.clsGlobal.myFunction.IdentityType);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.View)
                        {
                    ...
                    break;
                case "barNext":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "1", GTSysOne.Class.Utility.clsGlobal.myFunction.IdentityType);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {   
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.View)
                        {
                    ...
                    break;
                case "barLast":
                    ResultId = GTSysOne.Class.Utility.clsUtility.Navigator(Id, "3", GTSysOne.Class.Utility.clsGlobal.myFunction.IdentityType);
                    if (ResultId.CompareTo(string.Empty) != 0)
                    {
                        if (EnformStatus != GTSysOne.Gui.MasterFile.frmIdentityType.enFormStatus.View)
                        {
